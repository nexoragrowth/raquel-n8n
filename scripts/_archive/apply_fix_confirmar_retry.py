"""
Fix retry-loop en Sub-Agent Confirmar.

Bug detectado en shadow (execId 16154): el sub-agent llama `escalar_a_secretaria`
10 veces seguidas hasta `Agent stopped due to max iterations.`

Causa: el LLM no entiende que despues de llamar escalar_a_secretaria una vez,
tiene que devolver el canned y terminar. Sigue intentando llamar la tool.

Fix: agregar regla explicita al final del bloque especifico del prompt.
"""
import json
import os
import sys
...
def main():
    ...
    backup_path = f"workflows/history/v6_PRE_FIX_CONFIRMAR_RETRY_{ts}.json"
    with open(backup_path, "w", encoding="utf-8") as f:
        json.dump(wf, f, ensure_ascii=False, indent=2)
    print(f"  backup -> {backup_path}")

    node = next((n for n in wf["nodes"] if n["name"] == "Sub-Agent Confirmar"), None)
    if not node:
        sys.exit("ABORT: Sub-Agent Confirmar no encontrado")
    ...
    print(f"  Sub-Agent Confirmar: {len(sm)} -> {len(new_sm)} chars (+{len(new_sm)-len(sm)})")

    if DRY_RUN:
        out = f"workflows/history/v6_FIX_CONFIRMAR_RETRY_DRY_{ts}.json"
    ...
    for n in wf2["nodes"]:
        if n["name"] == "Sub-Agent Confirmar":
            assert MARKER in n["parameters"]["options"]["systemMessage"]
            print(f"  verified regla aplicada (active={wf2['active']})")
            break

    post_path = f"workflows/history/v6_POST_FIX_CONFIRMAR_RETRY_{ts}.json"
