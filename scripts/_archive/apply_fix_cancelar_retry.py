"""
Aplica la REGLA CRITICA - UNA SOLA ESCALACION POR TURNO al Sub-Agent Cancelar.

Mismo fix que se aplico a Sub-Agent Confirmar (apply_fix_confirmar_retry.py)
para evitar el retry-loop de `escalar_a_secretaria`.
"""
import json
import os
import sys
...
def main():
    ...
    backup_path = f"workflows/history/v6_PRE_FIX_CANCELAR_RETRY_{ts}.json"
    with open(backup_path, "w", encoding="utf-8") as f:
        json.dump(wf, f, ensure_ascii=False, indent=2)
    print(f"  backup -> {backup_path}")

    node = next((n for n in wf["nodes"] if n["name"] == "Sub-Agent Cancelar"), None)
    if not node:
        sys.exit("ABORT: Sub-Agent Cancelar no encontrado")
    ...
    print(f"  Sub-Agent Cancelar: {len(sm)} -> {len(new_sm)} chars (+{len(new_sm)-len(sm)})")

    if DRY_RUN:
        out = f"workflows/history/v6_FIX_CANCELAR_RETRY_DRY_{ts}.json"
    ...
    for n in wf2["nodes"]:
        if n["name"] == "Sub-Agent Cancelar":
            assert MARKER in n["parameters"]["options"]["systemMessage"]
            print(f"  verified (active={wf2['active']})")
            break

    post_path = f"workflows/history/v6_POST_FIX_CANCELAR_RETRY_{ts}.json"
