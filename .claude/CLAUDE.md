...
- **Round 11 (1/7)**: **Sub-WF Cancelar robustez 3-en-1** (autorizado Lucas "que sea ya", se iba a Italia). Aplicado a prod en 1 PUT atomico + backup pre/post + `node --check` de sintaxis en los 4 nodos tocados. Script: `apply_fix_cancelar_robustez_3en1.py`. Backups: `subwf_cancelar_PRE/POST_ROBUSTEZ3_20260701_144331.json`.
  1. **[ALTO] multi-fichas dead-end (caso Luis Augusto 1/7)**: familia con telefono compartido pide reprogramar, bot pide DNI, el paciente lo da y matchea su ficha correcta, PERO Step 4 (ramas `dni_match`/`context_match`) devolvia `decision:null` SIN setear `turno_objetivo` -> Step 5 (`const turno=prev.turno_objetivo`) veia vacio -> `if(!turno)` -> escalaba "No veo turno activo" con "sin razon", teniendo turno valido (cita 8163, vie 3/7). Fix: helper `__pickTurno` engancha el turno CONSERVADOR (solo si turnos_proximos pertenecen a la ficha resuelta = pacientesAll[0]; exige match de fecha si el paciente la menciono; si no puede con seguridad -> escala CON razon clara). Cero riesgo de reprogramar turno equivocado. Offline 6/6. Residual documentado: si la ficha resuelta != pacientesAll[0], no re-fetchea turnos (escala con razon) — mejora futura necesita E2E (falta `EVOLUTION_WEBHOOK_SECRET` local).
  2. **[ALTO] error tecnico -> escalar** (Steps 1b/2b/4): error de red/DNS (ej `EAI_AGAIN`, caso Agustin 22/6) dejaba de traducirse en "no tenes turnos" (mentira). Ahora `{error}` (continueOnFail) se distingue de `{data:[]}` -> escala. Offline 6/6.
  3. **[MEDIO] identidad Asiri** (Step 7): el Cancelar (codigo, sin LLM) nunca se presentaba -> pacientes le decian "que te fumaste" a la secretaria. Ahora antepone "Soy Asiri, la secretaria virtual..." en primer contacto/saludo/post-recordatorio/post-humano (no en continuaciones). Offline 7/7.
  - **"Razon: sin razon"** en las escalaciones del grupo = sintoma del bug #1 (Step 6c-prep hace `Razon: ' + (decision?.razon || 'sin razon')`; las ramas que escalaban sin setear razon caian al default). Los 3 fixes lo resuelven.
  - **Bug de observabilidad detectado (weekly-learning):** el reporte semanal del bot ALUCINA ("agrego 3 items al KB" = FALSO, KB sin cambios desde 5/6, agent_learnings vacia) y mis-categoriza (bot se loguea `rol=assistant` no `ai`; `rol=human`=0; "557 escalaciones" no validable). Recomendado a Lucas: gatear el auto-write al KB (propose-not-apply), arreglar el logging (rol consistente + tag escalacion) ANTES de medir, reemplazar narrativa-LLM por el funnel grounded del PDF `docs/` (Analisis-Producto-Asiri). PDF exhaustivo generado 24/6 (funnel + calidad + confiabilidad, observer L1/L2).
  - **Git**: sesion pusheada a branch `fix/cancelar-robustez-2026-07-01` (scripts + CLAUDE.md; workflows/ gitignored quedan locales).
